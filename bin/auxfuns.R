...
calc.GC.in.a.window.2 = function(start, sequence, window.size) return(GC(sequence[start:(start+window.size)]))

calculate.GC.in.windows.2 = function(windows.starts, sequence, bin.size = 0)
{
...
create_table = function(data, headers = NULL, x_offset = 1, y_offset = 0.5, 
                        col_width = 1, row_height = 0.5, colours = "black",
                        font_size = 1) 
{
  ...
  for (i in 1:num_rows) 
  {
    for (j in 1:num_cols) 
    {
      text(j * col_width - col_width / 2 + x_offset - col_width / 2, 
           ...
           labels = as.character(data[i, j]), cex = font_size, col = colours[i])
    }
  }
}
